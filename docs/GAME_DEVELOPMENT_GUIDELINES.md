...
> **최종 갱신**: 2026-09-02 (doodle-guess 네트워크·서버 로직 부록 추가)
...
#### doodle-guess 추가 패턴 (다중 방 실시간 동시 입력 게임 전용)

| 패턴 | 구현 |
|------|:----:|
| 3자리 방번호 / 재접속 복구 / 상태 머신 / 비율 CSS / Safe Area / 고정 헤더 / PWA | ✅ |
| **방별 FIFO 명령 큐** (동시성 단위 = 방, 큐 용량 초과 시 `SERVER_BUSY`) | ✅ |
| **드로잉 3중 좌표** (`drawingRevision` / `drawerEpoch` / `drawingSeq` + 획별 `batchSeq`) | ✅ |
| **백프레셔 계층 격리** (`bufferedAmount` 임계별 델타 중단·스냅샷 중단·연결 종료) | ✅ |
| **스냅샷 재동기화** (청크 + 조각 + SHA-256 검증, 복구 경로 단일화) | ✅ |
| **`requestId` 멱등성** (재전송 무시 + 저장된 응답 재전송) | ✅ |
| **레이트리밋 2중 버킷** (연결별 주 버킷 + 공유 IP 총량 버킷) | ✅ |
| **세션 토큰 회전 + liveness probe 슬롯 인계** | ✅ |
| **서버 시간 동기화** (`/api/time` RTT 최소 표본 + 단조 시계 앵커) | ✅ |
| **공개/개인 상태 팬아웃 분리 + 서버 계산 `allowedActions`** | ✅ |
| **부하 계측 엔드포인트** (이벤트 루프 지연, 방 큐 히스토그램, RSS 추이) | ✅ |
| 1,500 동시 연결 30분 무결성·지연 검증 | ✅ |
| 호스트 이양 | ⬜ (호스트 이탈 시 방 종료 정책 선택) |
| 서버 재시작 복구(스냅샷 영속화) | ⬜ (재시작 시 방 소멸을 명시적으로 선택) |

세부 구현은 **부록: Doodle-Guess 적용 사례** 참고.
...
| **doodle-guess** | `server/src/index.ts` | `server/src/game/gameService.ts` + `drawing/drawingService.ts` | `server/src/rooms/roomService.ts` + `roomCommandQueue.ts` | `client/src/state/GameContext.tsx` + `realtime/` | `client/src/styles/` |
...
## 부록: Doodle-Guess 적용 사례 — 다중 방 실시간 벡터 드로잉 서버

> **출처**: doodle-guess 프로젝트(2026-07 ~ 08) 설계·구현·부하 검증 결과를 일반화한 것. **초당 수십 건의 입력이 방마다 동시에 쏟아지는 실시간 게임**(드로잉, 경합형 정답, 실시간 협동)의 **네트워크·서버 로직**을 설계할 때 참고.
>
> 기존 게임들이 "턴이 돌면 한 명만 액션"인 구조라 다루지 않았던 문제들 — 동시 입력 직렬화, 연속 스트림의 순서 무결성, 느린 수신자 격리, 클라이언트 타이머 신뢰 — 을 다룬다.
>
> 검증 규모: 단일 Node 프로세스에서 **1,500 동시 연결(50방 × 30명) 30분** 무결성·지연 기준 통과.

### A. 방별 FIFO 명령 큐 — 동시성 제어의 기본 단위

- **배경**: 정답 선착순 판정, 그림 담당자 교체, 강퇴, 라운드 만료 타이머가 **같은 방에서 동시에** 실행되면 "두 명이 동시에 1등", "권한이 넘어간 뒤 이전 담당자의 획이 들어옴" 같은 경합이 생긴다. Node는 싱글 스레드지만 `await`가 하나라도 끼면 상태 검사와 상태 변경 사이가 갈라진다.
- **원칙**: **방 하나 = 직렬 실행 단위**. 방 상태를 건드리는 모든 명령(클라이언트 이벤트, 만료 타이머 콜백, 연결 종료 처리)을 그 방의 FIFO 큐에 넣어 한 번에 하나만 실행한다. 방끼리는 서로 대기하지 않으므로 방이 늘어도 지연이 전파되지 않는다.
- **구현**: 큐는 `Promise` 체인 하나로 충분하다. 별도 라이브러리 불필요.
...
class RoomCommandQueue {
  ...
  enqueue(task) {
    if (this.pending >= QUEUE_CAPACITY) {          // 방당 200
      return Promise.reject(new ProtocolError('SERVER_BUSY', '방이 잠시 혼잡합니다.'));
    }
    this.pending += 1;
    const result = this.tail.then(() => task());
    this.tail = result.then(() => undefined, () => undefined);  // 실패해도 체인 유지
...
- **필수 세부**:
  - `tail`은 **실패를 삼켜야 한다**. 한 명령이 던진 예외로 체인이 rejected 상태가 되면 그 방의 이후 명령이 전부 죽는다.
  - 큐 용량을 두고 초과분은 `SERVER_BUSY`로 **즉시 거절**한다. 무한 적재는 지연을 메모리 사용량으로 바꿀 뿐이다.
  - **큐 진입 전에 조회한 room 객체를 큐 안에서 다시 쓰지 않는다.** 대기하는 동안 방이 닫혔을 수 있으므로 큐 안에서 레지스트리를 다시 조회하고, 해당 연결이 아직 그 방에 속하는지 재검증한다.

```js
await room.queue.enqueue(() => {
  const currentRoom = registry.get(connection.roomCode);        // 재조회
  if (currentRoom.connections.get(actorId) !== connection) {    // 재검증
    throw new ProtocolError('NOT_IN_ROOM', '현재 연결은 방에 속하지 않습니다.');
  }
  execute(currentRoom, actorId, command);
});
```

- **타이머도 큐로**: 라운드 만료 `setTimeout` 콜백이 큐 밖에서 실행되면 "정답 처리 중 만료"가 겹친다. 콜백 본체를 `room.queue.enqueue(...)`로 감싸고, 실행 시점에 **라운드 ID와 상태를 다시 확인**해 이미 끝난 라운드면 조용히 무시한다.

### B. 서버 권위 드로잉 스트림 — revision · epoch · seq 3중 좌표

- **배경**: 그림은 턴제 액션과 달리 **끊기지 않는 스트림**이다. 지연·재전송·권한 교체·전체 지우기가 섞이면 "지운 뒤 도착한 옛 획", "이전 담당자의 잔여 획", "중간이 빠진 획"이 캔버스를 오염시킨다. 늦게 들어온 사람도 같은 그림을 봐야 한다.
- **원칙**: 모든 드로잉 이벤트에 **세 개의 좌표**를 붙이고, 서버가 셋 다 검증한다. 하나라도 어긋나면 거절한다.

| 좌표 | 증가 시점 | 막는 사고 |
|------|-----------|-----------|
| `drawingRevision` | 캔버스 전체 지우기 | 지우기 이전 캔버스의 획이 새 캔버스에 섞임 |
| `drawerEpoch` | 그림 담당자 교체 | 권한을 잃은 사람의 잔여 획이 반영됨 |
| `drawingSeq` | 서버가 이벤트를 낼 때마다 | 순서 뒤바뀜·누락 탐지 |

- **스트로크 단위 검증**: 한 획은 여러 배치로 나눠 오므로 획마다 `batchSeq`를 0부터 연속으로 요구한다.
  - `batchSeq < 기대값` → 재전송이므로 **저장해 둔 이전 응답을 그대로 돌려준다**(멱등, §D).
  - `batchSeq > 기대값` → **중간이 비었다**. 에러만 던지면 클라이언트 캔버스가 영구히 어긋나므로, 서버가 **즉시 스냅샷을 밀어 넣고** `STROKE_SEQUENCE_GAP`을 응답한다.
  - 진행 중인 획의 **도구·색·굵기가 바뀌면 거절**(`STROKE_STYLE_MISMATCH`). 한 획 안에서 스타일이 바뀌는 것은 정상 입력이 아니다.
  - 이미 `isFinal` 처리된 획에 추가 배치가 오면 거절.
- **클라이언트 커서**: 수신 측도 `(roundId, drawingRevision, drawingSeq)` 커서를 들고 판정한다 — 같거나 낮으면 `IGNORE`, 정확히 +1이면 `APPLY`, 그보다 크면 `GAP`(스냅샷 요청). 낙관적 렌더링은 **자기가 그리는 획에만** 허용하고, 남의 획은 서버 이벤트로만 그린다.
- **자원 상한은 서버가 강제**: 배치당 점 64개, 획당 2,048점, 캔버스당 획 1,000개 / 점 50,000개 / 직렬화 4MB. 클라이언트를 믿지 않는다. 상한 도달 시 `DRAWING_LIMIT`으로 거절하되 **방은 유지**한다.

### C. 백프레셔 — 느린 한 명이 방 전체를 끌어내리지 않게

- **배경**: 지하철·저가 단말의 느린 수신자 한 명이 있으면 그 소켓의 송신 버퍼가 계속 쌓인다. 그대로 두면 서버 메모리가 그 사람 몫으로 부풀고, 이벤트 루프가 밀려 **같은 방의 나머지 29명까지 느려진다**.
- **원칙**: **연결마다 `bufferedAmount`를 보고 계층적으로 대응**한다. 드로잉 델타는 "지금 못 보내면 버리고 나중에 스냅샷으로 대체 가능한" 데이터이므로 가장 먼저 포기한다.

| 버퍼 임계 | 조치 |
|---|---|
| 256KB 이상 | **드로잉 델타 전송 중단**, 해당 연결에 `needsSnapshot` 표시 (상태·점수 이벤트는 계속 보냄) |
| 128KB 미만으로 회복 | 250ms 주기 스캐너가 감지해 **전체 스냅샷 1회 전송**으로 복구 |
| 512KB 이상 | 진행 중인 **스냅샷 전송 중단** (다음 회복 시점에 재시도) |
| 1MB 이상이 5초 지속 | `close(1013)` — 되살아날 가망이 없는 연결을 끊어 방을 보호 |

- **핵심 판단**: **스냅샷 포기 기준을 델타보다 늦게 잡는다**(512KB vs 256KB). 같은 값을 쓰면 스냅샷이 매번 중간에 끊기고 처음부터 재시도하느라 영영 완주하지 못하는 라이브락이 생긴다.
- **압축을 켜지 말 것**: `perMessageDeflate: false`. 작은 JSON 메시지가 초당 수백 건 오가는 워크로드에서 deflate는 CPU와 연결당 메모리(컨텍스트)만 먹고 이득이 없다.
- **불필요한 방 전체 브로드캐스트를 줄인다**: 획이 끝나 "되돌리기 가능" 상태가 되는 사실은 **그린 본인에게만** 필요하다. 이런 것까지 전체에 뿌리면 느린 연결의 큐를 채워 정작 필요한 점수·라운드 이벤트를 뒤로 민다. **"이 정보가 필요한 사람이 누구인가"를 이벤트마다 판정**해 유니캐스트/브로드캐스트를 나눈다.

### D. 스냅샷 재동기화 — 청크·조각·해시

- **원칙**: 새 참여자 입장, 재접속, 시퀀스 갭, 백프레셔 회복 — **네 경우 모두 같은 스냅샷 경로**로 복구한다. 상황별 복구 코드를 따로 만들면 반드시 어긋난다.
- **구현**:
  - 스냅샷은 `snapshotId` 하나로 묶인 **여러 청크**(청크당 64KB 상한)로 보낸다. 긴 획은 **512점 단위 조각(fragment)** 으로 쪼개 하나의 청크가 상한을 넘지 않게 한다.
  - 표준 형태(canonical) 직렬화의 **SHA-256을 함께 실어** 클라이언트가 조립 후 검증한다. 불일치면 폐기하고 재요청 — 반쯤 깨진 캔버스를 그리는 것보다 낫다.
  - 클라이언트는 조립 중 도착한 델타를 **버퍼에 쌓아 뒀다가** 스냅샷 적용 직후 순서대로 재생한다. 스냅샷 도착 대기는 5초로 제한하고 초과 시 재요청한다.
  - 청크 번호 범위, 청크 간 메타데이터 일치, 중복 청크 동일성, 조각 인덱스 연속성을 **수신 측이 전부 검증**한다.

### E. 요청 멱등성 — requestId

- **배경**: 재접속 직후 클라이언트는 자기 요청이 처리됐는지 모른다. 재전송은 필연이고, 그대로 두면 정답이 두 번 집계되거나 획이 두 번 그려진다.
- **원칙**: **모든 클라이언트 명령에 `requestId`(UUID)를 필수**로 하고, 서버는 연결별로 처리 완료된 ID를 기억해 **재전송을 조용히 무시**한다. 보관은 60초면 충분하고, 매 메시지 처리 시 만료분을 정리한다.
- **응답이 필요한 재전송은 재응답한다**: 드로잉 배치는 `(roundId:revision:strokeId:batchSeq)` 키로 **직전 응답 이벤트를 저장**해 두고 같은 배치가 다시 오면 그 이벤트를 그대로 돌려준다. 추측(정답 시도)도 `guessId`로 같은 처리를 한다. "무시"만 하면 클라이언트가 영원히 기다린다.
- **에러 응답에도 `requestId`를 실어** 클라이언트가 어떤 요청이 실패했는지 매칭할 수 있게 한다.

### F. 레이트리밋 — 연결별 버킷과 공유 IP 버킷의 분리

- **배경**: 이 부류 게임은 **교실·사무실에서 한 회선으로 30명이 접속**한다. IP 기준 레이트리밋만 두면 정상 참여자가 서로를 밀어낸다. 반대로 연결 기준만 두면 한 사람이 소켓을 계속 새로 열어 방번호를 긁을 수 있다.
- **원칙**: **두 버킷을 동시에 운용하되 역할을 나눈다.**
  - **연결별 버킷** — 이벤트 종류마다 개별 한도. 개인의 연타를 막는 주 방어선.
  - **공유 IP 버킷** — 방 입장/개설에만 적용하고, **한 회선 전체의 총량**만 제한한다(정원 30명이 각자 몇 번 재접속해도 남을 만큼 넉넉하게: 개설 30회/분, 입장 120회/분).
- **한도는 실제 입력 속도에서 역산한다**: 드로잉 배치 25회/초(버스트 40), 추측 4회/초, 방 개설 5회/분처럼 **이벤트마다 다르게** 잡는다. 하나의 전역 한도는 드로잉을 막거나 방 개설을 열어두거나 둘 중 하나로 실패한다.
- **프록시 뒤 IP**: `X-Forwarded-For`는 **신뢰하는 프록시 뒤에서만**(`TRUST_PROXY` 명시적 설정) 첫 항목을 사용한다. 이 값은 보조 버킷 키로만 쓰므로, 위조해도 연결별 주 버킷은 우회할 수 없다 — **위조로 얻을 이득이 없는 구조**로 만들어 두는 것이 검증보다 견고하다.

### G. 세션 토큰 회전과 liveness probe 기반 슬롯 인계

- **배경**: 재접속 시 "토큰이 맞으면 기존 연결을 끊고 인계"는 위험하다. 토큰을 훔친 쪽이 **정상 접속자를 계속 쫓아낼 수 있다**. 그렇다고 "이미 접속 중이면 거절"하면, 죽은 소켓이 하트비트 타임아웃(90초)까지 `connected`로 남아 **정작 본인의 0.5초 뒤 재접속이 막힌다**.
- **원칙**: 인계 전에 **기존 연결의 생존을 먼저 확인**한다.
  1. 첫 시도 → 기존 연결에 `ping`을 보내고 `SESSION_IN_USE`로 "확인 중"을 응답 (클라이언트는 백오프 후 재시도)
  2. 유예(1초) 안에 `pong`이 오면 → **살아 있는 연결이므로 보호**, 재접속 거절
  3. 유예를 넘겨도 응답이 없으면 → 죽은 연결로 판정, 슬롯 인계 후 `close(4004)`
- **토큰 회전**: 세션 토큰은 32바이트 난수, 서버에는 **SHA-256 해시만** 저장하고, **입장·재입장할 때마다 새로 발급**한다. 유출된 토큰의 유효 기간을 짧게 만든다.
- **강퇴는 닉네임과 토큰 해시를 모두 기록**해 재입장을 막는다. 둘 중 하나만 막으면 닉네임을 바꾸거나 토큰을 지워 우회된다.
- **클라이언트 재입장 백오프**: 재연결은 `500ms × 2^n`(최대 5초), 재입장 요청은 `400ms × 2^n`(최대 4초, 5회)로 지수 백오프한다. **재시도 가능한 에러 코드 목록을 명시**해(`SESSION_IN_USE`, `SERVER_BUSY`, `RATE_LIMITED` 등) 그 외에는 즉시 포기하고 사용자에게 알린다.

### H. 서버 시간 동기화 — 클라이언트 타이머를 신뢰 가능하게

- **배경**: 남은 시간을 매초 브로드캐스트하면 방당 30명 × 초당 1회의 낭비다. 그렇다고 클라이언트가 `Date.now()`로 계산하면 **기기 시계 오차만큼 다른 시간이 보인다**.
- **원칙**: 서버는 **종료 시각(`roundEndsAt`)만 절대값으로 한 번** 내려보내고, 클라이언트가 **서버 시계에 맞춰 보정한 값**으로 매초 스스로 센다. 판정은 언제나 서버가 한다.
- **구현**: `/api/time`이 `serverReceivedAt`/`serverSentAt`을 반환하고, 클라이언트는 **3회 표본 중 RTT가 가장 작은 것**을 앵커로 채택한다(NTP 방식의 축소판). 이후 경과 시간은 `Date.now()`가 아니라 **단조 시계 `performance.now()`** 로 재 — 절전·시계 조정으로 벽시계가 튀어도 타이머가 뛰지 않는다.
- **표본 유효성 검사**: 벽시계 경과와 단조 시계 경과의 차이가 100ms를 넘는 표본은 버린다(측정 중 시계가 조정된 경우).
- **서버 측 이중 방어**: 만료 타이머와 별개로, **모든 라운드 명령의 진입점에서 마감 시각을 재확인**한다. 타이머가 늦게 깨어난 사이 들어온 입력을 받아주면 안 된다.

### I. 팬아웃 분리 — 공개 상태와 개인 상태

- **원칙**: 상태를 **`PUBLIC_STATE`(방 전체 동일)와 `PRIVATE_STATE`(수신자별로 다름)로 완전히 분리**한다. 하나의 상태에 개인 정보를 담고 클라이언트에서 가리는 방식은 DOM/네트워크 탭에서 그대로 뚫린다.
- **제시어 노출은 서버가 판정**: "제시어를 본 사람" 집합(`keywordExposedPlayerIds`)을 서버가 관리하고, 담당자 교체·강퇴로 제시어가 넘어갈 때마다 갱신한다. **이 집합에 든 사람은 추측 자격을 잃는다** — 권한 판정과 정보 은닉을 같은 자료구조로 처리하면 둘이 어긋나지 않는다. 단 **라운드가 끝난 뒤의 교체까지 이 집합에 넣으면 안 된다**. 끝난 라운드의 정답은 이미 공개된 것이라 지킬 대상이 아닌데, 넣어 버리면 다음 라운드 시작 조건이 엉뚱하게 막힌다. 조건은 `hasKeyword`가 아니라 "라운드가 실제로 돌고 있는가"로 잡는다.
- **정답을 아는 사람이 권한을 넘길 때 제시어를 갈아 끼워라**: 방장이 기본 그리기 담당자인 구조에서는, 방장이 제시어를 본 뒤 남에게 넘기면 자기가 답을 아는 채로 추측하게 된다. 해결은 두 가지를 합쳐야 한다. ① 제시어를 **기본 가림**으로 두어 보지 않고도 넘길 수 있게 하고, ② 실제로 열어 본 사실을 서버에 신고받아(`REVEAL_KEYWORD` 같은 전용 이벤트) 넘길 때 제시어를 새로 뽑는다. **클라이언트의 보기/가리기 토글만으로는 서버가 알 수 없다** — 화면 로컬 상태로 두면 이 판정을 할 수 없으니 처음부터 신고 이벤트를 설계에 넣어라.
- **허용 액션 목록을 서버가 계산해 내려준다**: 클라이언트가 자기 판단으로 버튼을 활성화하지 않는다. 서버가 `allowedActions` 배열을 개인 상태에 실어 주고, 클라이언트는 그대로 렌더링만 한다. 서버는 액션 실행 시 **다시 한번 권한을 검증**한다(목록은 UI용, 검증은 별도).

### J. 프로토콜 하드닝 체크리스트

- **메시지 크기 3중 상한**: `ws`의 `maxPayload`(16KB) → 파싱 전 바이트 검사 → **이벤트 종류별 상한**(드로잉 배치 8KB, 나머지 1~2KB). 종류별 상한이 없으면 "16KB짜리 닉네임"이 통과한다.
- **바이너리 프레임 거절**: 텍스트 JSON만 받는다(`close(1003)`).
- **업그레이드 게이트**: 경로가 `/ws`가 아니거나 Origin이 허용 목록에 없으면 **핸드셰이크 단계에서 403**으로 끊는다. 종료 중(`acceptingUpgrades = false`)에도 막는다.
- **에러는 코드 체계로**: 40여 개의 명시적 에러 코드(`STALE_DRAWING_REVISION`, `SESSION_IN_USE`, `SHUFFLE_LIMIT` …)를 정의하고 클라이언트가 **코드로 분기**하게 한다. 메시지 문자열로 분기하면 문구를 바꾸는 순간 깨진다.
- **하트비트**: 45초 주기 `ping`, 90초 무응답 시 `terminate()`. 타이머에는 `unref()`를 걸어 종료를 막지 않게 한다.
- **Graceful shutdown**: 업그레이드 차단 → 모든 방에 `ROOM_CLOSED(SERVER_SHUTDOWN)` 통보 → `close(1012)` → HTTP 종료(5초 강제 타임아웃).

### K. 부하 검증 — 무엇을 재고, 어디서 떨어지는가

- **원칙**: 실시간 게임의 부하 시험은 **처리량이 아니라 정합성과 꼬리 지연**을 본다. 아래 항목은 하나라도 0이 아니면 설계 결함이다.

| 측정 항목 | 합격 기준 | 실측(600연결) | 실측(1,500연결) |
|---|---:|---:|---:|
| 정상 연결 끊김 / 예상 밖 오류 | 0 | 0 | 0 |
| 드로잉 시퀀스 갭 / 중복 이벤트 / 타 방 유출 | 0 | 0 | 0 |
| stroke 반영 p95 / p99 | <150 / <300ms | 12 / 15ms | 28 / 60ms |
| 정답 확정 p95 / p99 | <200 / <400ms | 14 / 29ms | 72 / 140ms |
| 방 큐 대기 p95 | <50ms | 1ms | 1ms |
| 이벤트 루프 지연 p95 | <50ms | 15.9ms | 25.0ms |
| CPU (1코어 대비) | <70% | 20.5% | 44.0% |
| 워밍업 대비 RSS 증가 | <10% | +10.24% ❌ | -13.58% |

- **시나리오에 반드시 포함할 것**:
  - **의도적으로 느린 수신자**를 방마다 1명 섞는다(20초 수신 정지 → 5초 복구 반복). 백프레셔 경로는 이렇게 하지 않으면 절대 실행되지 않는다.
  - 30분 이상 **장시간** 유지 — 메모리 누수는 짧은 시험에서 안 보인다. **워밍업 5분 후를 기준선**으로 잡아야 초기 할당과 누수를 구분할 수 있다.
  - 정답 **동시 경합**과 라운드 반복 전환.
- **계측 엔드포인트를 미리 만든다**: 이벤트 루프 지연(`monitorEventLoopDelay`), 방 큐 대기 히스토그램, RSS 추이를 `/health/load-metrics`로 노출하고 **환경 변수로 켜고 끈다**. 사후에 붙이면 재현이 어렵다.
- **판정은 기계적으로**: 위 실측에서 600연결 RSS가 기준을 0.24%p 초과했다. 1,500연결에서는 오히려 감소해 단조 누수로 볼 근거는 없었지만, **단일 시점의 측정값을 임의로 합격 처리하지 않았다.** 기준을 세웠으면 애매한 값은 미충족으로 기록한다.

### L. 서버 재시작 정책은 미리 못박는다

- **배경**: 인메모리 방 구조에서 "재시작 시 어떻게 되는가"를 정하지 않으면 클라이언트가 유령 방으로 무한 재접속한다.
- **원칙**: 이 게임은 **재시작 시 모든 방 소멸**을 명시적으로 택했다(DB·Redis 없음, 단일 replica). 대신 클라이언트가 `ROOM_NOT_FOUND`를 받으면 **저장된 세션을 즉시 폐기하고** "서버가 다시 시작되어 기존 방이 종료되었습니다"로 안내한다.
- **단일 replica 강제**: 방 상태가 프로세스 메모리에 있으므로 스케일 아웃하면 같은 방번호가 서로 다른 인스턴스에 생긴다. **배포 설정에 replica=1을 문서로 못박고**, 필요해지는 시점에 방-인스턴스 라우팅 또는 외부 저장소를 도입한다. §2.12의 스냅샷 복구를 쓸지도 이 시점에 함께 결정한다.

### M. 낙관적 렌더링의 인계 — 펜을 떼는 순간을 끊지 마라

> **두 게임에서 반복된 버그다.** doodle-dash(2026-09-10)와 doodle-guess(2026-09-13)가 같은 실수를 했다. 실시간 드로잉을 새로 만든다면 이 절을 먼저 읽어라.

- **증상**: 획을 그리고 펜을 떼면 그 획이 **처음부터 다시 그려지는 것처럼** 보인다. 사라졌다 나타나는 깜빡임으로 보이기도 한다.
- **원인**: 그리는 동안 화면에 보이는 것은 **로컬 낙관 획**이고, 최종적으로 남는 것은 **서버 확정본**이다. 펜을 떼는 순간 낙관 획을 치웠는데 확정본이 아직 자리에 없으면 그 사이가 비어 버린다.
- **비는 시간이 구조에 따라 다르다.**
  - 확정본을 **로컬에서 즉시** 만들어 넣는 구조(doodle-dash): 비는 구간은 **한 프레임**. 그래도 눈에 띈다.
  - 확정본이 **서버 왕복 후**에 오는 구조(doodle-guess, `drawingSeq` 서버 권위): 비는 구간이 **왕복 시간 전체**. 배치를 120ms마다 나눠 보내면 뒷부분이 배치 단위로 하나씩 채워져 **정말로 다시 그려지는 것처럼** 보인다.
- **원칙**: **낙관 획은 확정본이 실제로 자리에 들어온 뒤에 치운다.** 펜을 떼는 순간이 아니다.
  - 로컬 확정 구조: 그 자리에서 다시 그리지 **않는다**. 확정본이 목록에 들어오면 상태를 보는 효과가 어차피 다시 그린다.
  - 서버 왕복 구조: 뗀 획을 따로 들고 있다가 확정본(`finalized`)이 도착하면 비운다. 되돌리기·전체 지우기·라운드 교체·권한 상실 때도 비워 유령 획이 남지 않게 한다.
- **미리보기는 획을 시작할 때 잡아 둔 도구 설정으로 그린다.** 렌더 시점의 현재 설정을 쓰면 획 도중 도구를 바꿨을 때 이미 그린 획의 색·굵기가 따라 변하고, 서버로 보낸 값과도 어긋난다.
- **지우개는 이 방법으로 못 덮는다.** 미리보기 레이어는 투명해서 `destination-out`이 지울 대상이 없다. 지우개 획은 본 캔버스에서만 반영되므로 같은 깜빡임이 남는다. 필요하면 별도 설계가 든다.
- **로컬에서는 거의 재현되지 않는다.** 왕복이 0에 가까워 비는 구간이 안 보인다. **실기기·배포 서버에서 확인**해야 한다.

> **요약 — 실시간 동시 입력 게임의 서버 설계 시 참고**
> - 동시성 단위는 **방**: 방별 FIFO 큐로 직렬화, 큐 안에서 방과 연결을 **재조회·재검증**, 타이머 콜백도 큐로
> - 연속 스트림은 **revision · epoch · seq 3중 좌표**로 검증, 갭이면 **서버가 먼저 스냅샷을 밀어 넣는다**
> - 느린 수신자는 **버퍼 임계로 계층 격리**하고, **스냅샷 포기 기준은 델타보다 늦게** 잡는다(라이브락 방지)
> - 모든 명령에 **`requestId` 멱등성**, 응답이 필요한 요청은 **저장된 응답을 재전송**
> - 레이트리밋은 **연결별(주) + 공유 IP(총량 보조)** 2중 — 한 회선 30명 접속을 전제로 한도를 잡는다
> - 재접속 슬롯 인계는 **ping/pong 생존 확인 후**에만, 세션 토큰은 **매 입장 회전**
> - 타이머는 **종료 시각 1회 전송 + 클라이언트 보정(RTT 최소 표본 · 단조 시계)**, 판정은 서버가 재확인
> - 부하 시험은 **정합성 0건 + 꼬리 지연**을 본다. **느린 수신자를 반드시 섞고**, 30분 이상, 워밍업 5분 기준선
> - 인메모리 구조는 **재시작 정책과 단일 replica 제약을 문서로 못박는다**
> - 낙관 획은 **확정본이 자리에 들어온 뒤** 치운다 — 펜을 떼는 순간 치우면 "획이 처음부터 다시 그려진다"(두 게임에서 반복된 버그)
