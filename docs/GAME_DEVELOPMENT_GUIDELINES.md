...
- **제시어 노출은 서버가 판정**: "제시어를 본 사람" 집합(`keywordExposedPlayerIds`)을 서버가 관리하고, 담당자 교체·강퇴로 제시어가 넘어갈 때마다 갱신한다. **이 집합에 든 사람은 추측 자격을 잃는다** — 권한 판정과 정보 은닉을 같은 자료구조로 처리하면 둘이 어긋나지 않는다.
